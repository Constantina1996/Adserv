...
prove_with_tree(Query, Delta, Tree) :-
	...
	(contains_term('NO COUNTERATTACK', Tree) ->
	 Delta = 'FAIL';
	 Delta = Delta1).

%% nbassili
%% Returns also the decision tree
prove_wTree(Query, Delta, Tree) :-
...
extend_wTree(Delta0, DeltaAcc, Delta, ReturnTree) :-
	isconsistent(Delta0),
...
counterattackone_wTree(AttackNode, DeltaThis, DeltaThis, 'NO COUNTERATTACK'-[]) :-
...
print_label( [], 0, Label ) :- !,
	...
	nl.
print_label( [_], 1, Label ) :- !,
	write('|___'),
	write_arg_label( Label ), 
	nl.
print_label( [Order|Rest], Level, Label ) :- !,
	draw_lines(Order),
	NextLevel is Level - 1,
	print_label(Rest, NextLevel, Label).
...
ps_tree( Order, Level, Label-[], String) :- !, % Print a leaf.
    string_label( Order, Level, Label, String ).
ps_tree( Order, Level, Label-[[]], String ) :- !, % Print a leaf.
    string_label( Order, Level, Label, String ).
ps_tree( Order, Level, Label-SubTrees, String ) :- !,
    SubTrees = [_|_], 
    string_label( Order, Level, Label, S1 ), 
    NewLevel is Level + 1, 
...
% print_label( Order, Level, Label )
string_label( [], 0, Label, String ) :- !,
	string_arg_label( Label, String1 ),
	atom_concat(String1,"\n",String).
string_label( [_], 1, Label, String ) :- !,
	string_arg_label( Label, String1 ), 
	atomic_list_concat(['|___',String1,"\n"], String).
string_label( [Order|Rest], Level, Label, String ) :- !,
	string_lines(Order,Str1),
	NextLevel is Level - 1,
	string_label(Rest, NextLevel, Label, Str2),
